...
class Collection extends \Magento\Catalog\Model\ResourceModel\Product\Collection
{
    ...
    {
        parent::__construct(
            $entityFactory, $logger, $fetchStrategy, $eventManager, $eavConfig, $resource, $eavEntityFactory, $resourceHelper, $universalFactory, $storeManager, $moduleManager, $catalogProductFlatState, $scopeConfig, $productOptionFactory, $catalogUrl, $localeDate, $customerSession, $dateTime, $groupManagement, $connection, $productLimitationFactory, $metadataPool
        );
    ...
    public function getGroupedProducts(
        $storeId, $notLike, $concat, $listOfAttributes
    )
    {
        ...
        $this->getSelect()
            ->joinLeft(["cpl" => $tableCpl], "cpl.product_id=e." . $this->_rowId . " AND cpl.link_type_id=3", ["child_ids" => "GROUP_CONCAT( DISTINCT cpl.linked_product_id)"])
            ...
            ->joinLeft(["categories" => $tableCcp], "categories.product_id=e.entity_id", ["categories_ids" => "GROUP_CONCAT( DISTINCT categories.category_id)"])
    ...
    protected function getParentProducts(
        $type, $storeId, $notLike, $concat, $listOfAttributes
    )
    {
        ...
        if (class_exists("\Magento\Catalog\Model\Indexer\Category\Product\TableMaintainer")) {
            ...
            $tableCcpi = $this->_resource->getTableName($indexTableName);
        }
        $tableCpsl = $connection->getTableName("catalog_product_super_link");
        $tableCur = $connection->getTableName("url_rewrite");
    ...
    public function getProductCount(
        $storeId, $websiteId, $notLike, $concat, $manageStock, $listOfAttributes, $categoriesFilterList,
        $condition, $params
    )
    {
    ...
    )
    {
        ...
        if ($categoriesFilterList[0] != "*") {
            ...
            $this->getSelect()->joinInner(["categories" => $tableCcp], $ct . $filter, ["categories_ids" => "GROUP_CONCAT( DISTINCT categories.category_id)"]);
            $this->getSelect()->joinInner(["cce" => $tableCce], "categories.category_id=cce.entity_id AND cce.path LIKE '" . $categoryRootId . "/" . $storeRootId . "/%'", []);

        } else {
            $this->getSelect()->joinLeft(["categories" => $tableCcp], "categories.product_id=e.entity_id", ["categories_ids" => "GROUP_CONCAT( DISTINCT categories.category_id)"]);
