...
namespace Sparx\Delivery\Model\ResourceModel\Carrier;
...
class Matrixrate extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    /**
     * @var \Sparx\Delivery\Model\Carrier\Matrixrate
     */
     ...
     * @param \Sparx\Delivery\Model\Carrier\Matrixrate $carrierMatrixrate
    ...
    public function __construct(
        ...
        \Sparx\Delivery\Model\Carrier\Matrixrate $carrierMatrixrate,
    ...
    public function getRate(\Magento\Quote\Model\Quote\Address\RateRequest $request, $zipRangeSet = false)
    {
        ...
        if($request->getDestCountryId() and $request->getDestPostcode()){
        ...
        for ($j=0; $j<8; $j++) {
            ...
            $zoneWhere='';
           // $bind=[];
            $zoneWhere =  "dest_country_id ='".$request->getDestCountryId()."' AND (dest_region_id = 0 or dest_region_id = '*') AND dest_city = '*' and (dest_zip <=".$request->getDestPostcode()." and dest_zip_to >= ".$request->getDestPostcode().")";
                    //~ $bind = [
...
//$this->logger->info('condition_value='.$bind[':condition_value'].' CId='.$bind[':website_id']);
          //  $select->where('condition_name = :condition_name');
            $select->where('condition_from_value <= '.$request->getData($request->getConditionMRName()));
            $select->where('condition_to_value >= '.$request->getData($request->getConditionMRName()));
            //$select->where('website_id >= '.(int)$request->getWebsiteId());

           // $this->logger->debug('SQL Select: ', $select->getPart('where'));
           // $this->logger->debug('Bindings: ', $bind);

            $results = $adapter->fetchAll($select);
			//$this->logger->info( $select->where($zoneWhere));
            if (!empty($results)) {
     ...
     * @return \Sparx\Delivery\Model\ResourceModel\Carrier\Matrixrate
