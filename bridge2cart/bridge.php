...
class M1_Bridge_Action_Savefile
{
  ...
  protected function _saveFile($source, $destination, $width, $height, $local = '')
  {
    ...
    if (isset($pathInfoSource['extension']) && strpos($source, 'https://images.shopping-cart-migration.com/index.php') !== 0) {
...
class M1_Config_Adapter_PrestaShop extends M1_Config_Adapter
{
  public function __construct()
  {
    ...
    if (file_exists(M1_STORE_BASE_DIR . '/app/config/config.yml') || file_exists(M1_STORE_BASE_DIR . '/app/config/parameters.yml')) {
      ...
      define('_DB_SERVER_', $config['parameters']['database_host'] . $port);
  ...
  public function connect()
  {
    ...
    while ($triesCount) {
      try {
        ...
        if (!empty($_GET['disable_checks']) && $forceExec) {
    ...
    if (isset($_REQUEST['tableName']) && isset($_REQUEST['data'])) {
      ...
      if ($result) {
        ...
        try {
          ...
          $res = $bridge->query('LOAD DATA INFILE "' . $file . '" IGNORE INTO TABLE ' . $_REQUEST['tableName'] . ' FIELDS TERMINATED BY \',\' ENCLOSED BY "\'"', (int)$_REQUEST['fetchMode']);
          if (!empty($_GET['disable_checks'])) {
