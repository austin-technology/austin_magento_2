...
class LECM_Connector_Adapter
{
    ...
    function detectCartType() {
        ...
        if (file_exists(LECM_STORE_BASE_DIR . 'wp-config.php')) {
            ...
            //phpurchase
            $PhpurchaseDir = glob(LECM_STORE_BASE_DIR . 'wp-content/plugins/phpurchase*', GLOB_ONLYDIR);
            if (is_array($PhpurchaseDir) && count($PhpurchaseDir) > 0){
                return 'phpurchase';
            }
            
            //WP Online Store
            $WPonlinestoreDir =  glob(LECM_STORE_BASE_DIR . 'wp-content/plugins/wp-online-store*', GLOB_ONLYDIR);
            if (is_array($WPonlinestoreDir) && count($WPonlinestoreDir) > 0){
                return 'wponlinestore';
            }
...
class LECM_Connector_Adapter_WPonlinestore extends LECM_Connector_Adapter{
    function __construct(){
        parent::__construct();
...
class LECM_Connector_Adapter_Phpurchase extends LECM_Connector_Adapter{
    function __construct(){
        parent::__construct();
...
class LECM_Connector_Adapter_Shopp extends LECM_Connector_Adapter {
    function __construct() {
        ...
        preg_match('/\$table_prefix\s*=\s*\'(.*)\'\s*;/', $config, $match);
        $this->tablePrefix = $match[1];
        $this->imageDir = 'shop/images/';
        $this->imageDirCategory    = $this->imageDir;
        $this->imageDirProduct      = $this->imageDir;
        $this->imageDirManufacturer = $this->imageDir;
        $this->version = $this->getCartVersionFromDb('value', 'shopp_meta', "name = 'version'");
    }
}
